           ...
           HOF  "MOT32"
           WDLN 8
; SYMPL GP-GPU Shader Demo 3D Transform Micro-Kernel(SYMPL IL version)
...
load_vects: 
            ...
            uh      INEXT_VECT = uh:#INEXT_
                    ...
                    setDone                                  ;to signal host thread has started (ie, not done)                
any_triangles?:

                    cmp(uh:triangles, uh:#0x0)
...
loop:   ;scale on X, Y, Z axis
            ;vertex 1
            fs        FMUL.0 = multiplication:(fs:*AR3++[4], fs:scaleX)
            fs        FMUL.1 = multiplication:(fs:*AR3++[4], fs:scaleY)
            fs        FMUL.2 = multiplication:(fs:*AR3++[4], fs:scaleZ)
            ;vertex 2
            fs        FMUL.3 = multiplication:(fs:*AR3++[4], fs:scaleX)
            fs        FMUL.4 = multiplication:(fs:*AR3++[4], fs:scaleY)
            fs        FMUL.5 = multiplication:(fs:*AR3++[4], fs:scaleZ)
            ;vertex 3
            fs        FMUL.6 = multiplication:(fs:*AR3++[4], fs:scaleX)
            fs        FMUL.7 = multiplication:(fs:*AR3++[4], fs:scaleY)
            fs        FMUL.8 = multiplication:(fs:*AR3++[4], fs:scaleZ)
  ...
  ;rotate around Z axis
    ...
    ;now translate on X, Y = Z axis
        ...
        ;vertex 3
            ...
            uh      triangles = uh:#0              ;clear triangles so it doesn't fall through again
