...
assign add_invalid = Rmult_infinity_del_2 && B_is_infinite_del && ((A_sign_del ^ B_sign_del) ^ C_sign_del) && wren_del_2;    


assign ready = readyA && readyB;

// this operator will never over/under-flow, due to the extra bit in the exponent and the guardbanding on the front end.
// It can, however, produce an inexact intermediate result.  If this happens, the result of the fatADD will also be inexact.
// In such event, the FP610_To_IEEE754 signals inexactness--not the fatMUL 
...
FP610_To_IEEE754_510_filtered FP610toIEEE510(
    ...
    .div_by_0_del      (1'b0         ),      
    .A_invalid_del     (A_invalid_del || C_invalid_del),      
    .B_invalid_del     (B_invalid_del),      
    .A_inexact_del     (A_inexact_del || mul_inexact_del_2),      
...
always @(posedge CLK or posedge RESET) begin
    if (RESET) semaphor <= 64'hFFFF_FFFF_FFFF_FFFF;
        ...
        if (wren_del_3) semaphor[wraddrs_del] <= 1'b1;
    end
end     
