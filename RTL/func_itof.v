...
assign itof_invalid = (~(&wrdata[63:16] || ~|wrdata[63:16]) || (~Sext_SrcA_q2 && (wrdata[15:0] > 16'hFFE0)));  //16'hFFE0 = 65504 

assign {guard, round, sticky} = {R[4], R[3], |R[2:0]};

assign ready = readyA && readyB;

//this operator is made fat to provide a bunch of extra bits to form guard, round and sticky bits to support directed rounding
FXP_To_FP615 FXP_To_FP615(      
    .clk (CLK),              
    .rst (RESET),            
    .I   ({Sext_SrcA_q2, wrdata[15:0]}),     //input is 16-bit signed or unsigned source with sign extend signal functioning as 17th bit for this "signed" operator
    .O   (R )               //result is FP615
    );
...
FP610_To_IEEE754_510_filtered FP610toIEEE510(
    ...
    .Away              (Away     ),
    .trunk_invalid     (itof_invalid ),
    .NaN_in            (10'b0        ), 
    .invalid_code      (quantize),      
