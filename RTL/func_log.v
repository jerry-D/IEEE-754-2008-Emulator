...
assign invalid = A_sign_del && wren_del_5 && ~A_is_zero_del; // A is a negative non-zero number (including infinity)
                   
assign div_by_0 = A_is_zero_del && wren_del_5 ;                  
...
FPLog613 FPLog613(
    ...
    .rst (RESET ),
    .X   ({wrdataA, 3'b0}),
    .R   (R613 )            //FP610 representation is 19 bits, which includes FloPoCo 2-bit exception code
    );
...
RAM_func #(.ADDRS_WIDTH(6), .DATA_WIDTH(18))
    ...
    .wren       (wren_del_6),
    .wraddrs    (wraddrs_del),
    .wrdata     ({R18[17:16], (div_by_0 || R18[15]), R18[14:0]}),      
    .rdenA      (rdenA   ),
    .rdaddrsA   (rdaddrsA),
    .rddataA    (rddataA ),
    .rdenB      (rdenB   ),
...
always @(posedge CLK or posedge RESET) begin
    if (RESET) semaphor <= 64'hFFFF_FFFF_FFFF_FFFF;
        ...
        if (wren_del_6) semaphor[wraddrs_del] <= 1'b1;
    end
end
