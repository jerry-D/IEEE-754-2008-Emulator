...
assign POP_PC_sext  = (OPsrcA_q2[2:0]==3'b111) && (OPdest_q2==PC_ADDRS[15:0]) && Ind_SrcA_q2 && Sext_Dest_q2 && thread_q2_sel && wrcycl;
assign PUSH_PC_sext = (OPdest_q2[2:0]==3'b111) && (OPsrcA_q2==PC_COPY_ADDRS[15:0]) && Sext_Dest_q2 && Ind_Dest_q2 && thread_q2_sel && wrcycl;
...
assign NMIg = NMIq;                                                                                                                                              
assign IRQg = IRQq;                                                                                                                                               
assign EXCg = (inexact_exc || underflow_exc || overflow_exc || divby0_exc || invalid_exc) && ~EXC_ack && ~EXC_in_service && thread_q1_sel;                                                                                                                                              
...
always @(posedge CLK or posedge RESET) 
 ...
 else NMIq <= thread_q0_sel && NMI && ~NMI_in_service;

always @(posedge CLK or posedge RESET) 
 if (RESET) IRQq <= 1'b0;
 else if (IRQ_ackq && (vector==IRQ_VECTOR)) IRQq <= 1'b0;
 else  IRQq <= (thread_q0_sel && IRQ && IRQ_IE && ~IRQ_in_service);
