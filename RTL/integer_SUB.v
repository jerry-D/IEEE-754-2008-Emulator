...
wire c_flag_withBorrow;
wire v_flag_withBorrow;

wire c_flag_withOutBorrow;
wire v_flag_withOutBorrow;

assign ready = readyA & readyB;  
assign wrenq = delay0[6];
assign wraddrsq = delay0[5:0];  
assign SUB_result = {(Cin && Sext_Dest_q3), oprndAq} - oprndBq;
assign z_flag = ~|SUB_result;
assign n_flag = SUB_result[64] && SextResult;
assign c_flag_withBorrow = (~oprndAq[64] && oprndBq[64]) || (oprndBq[64] && SUB_result[64]) || (~oprndAq[64] && SUB_result[64]);
assign v_flag_withBorrow = (oprndAq[64] &&  ~oprndBq[64] && ~SUB_result[64]) || (~oprndAq[64] &&  oprndBq[64] && SUB_result[64]);

assign c_flag_withOutBorrow = (oprndAq[64] && oprndBq[64]) || (oprndBq[64] && ~SUB_result[64]) || (oprndAq[64] && ~SUB_result[64]);
assign v_flag_withOutBorrow = (oprndAq[64] &&  oprndBq[64] && ~SUB_result[64]) || (~oprndAq[64] && ~oprndBq[64] && SUB_result[64]);

assign c_flag = Sext_Dest_q3 ? c_flag_withBorrow : c_flag_withOutBorrow;
assign v_flag = Sext_Dest_q3 ? v_flag_withBorrow : v_flag_withOutBorrow;
...
RAM_func #(.ADDRS_WIDTH(6), .DATA_WIDTH(68))
    ...
    .wren       (wrenq    ),
    .wraddrs    (wraddrsq ),
    .wrdata     ({c_flag, v_flag, n_flag, z_flag, SUB_result[63:0]}),
    .rdenA      (rdenA    ),
    .rdaddrsA   (rdaddrsA ),
    .rddataA    (rddataA  ),
    .rdenB      (rdenB    ),
